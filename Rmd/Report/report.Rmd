...
Principal Component Analysis (PCA) was applied as a way to be able to reduce the dimensionality of the dataset while retaining as much variance as possible. This technique helps in identifying key patterns in the data and visualizing its structure.

## 4.1 Methodology

After the initial data preprocessing and analysis, we've seen that the dataset contains numerical variables with different scales and given the nature of tha data we opted for no transformations as explained previously. Given this situation, our best course of action is to apply PCA to the dataset using the correlation matrix instead of the covariance matrix. This is because the variables have different scales, and using the correlation matrix will standardize the variables to have a mean of 0 and a standard deviation of 1, making them comparable.
